...
Our models are implemented in the R/C++ package **SpatialGEV** included in the Supplementary Material as `SpatialGEV_1.0.1.tar.gz`.  **SpatialGEV** provides tool to perform approximate inference for a number of generalized extreme value (GEV) models with spatially-varying parameters modelled via a Gaussian process, which we refer to as GEV-GP models.  
...
Now we install the **SpatialGEV** package by running the following from the R working directory containing `SpatialGEV_1.0.1.tar.gz`:
...
install.packages("SpatialGEV_1.0.1.tar.gz", repos = NULL, type = "source", 
...
big_a_est <- true_vs_est_scatterplot(
  big_sim$a, a_s,
  axis_lim = a_range,
  title = expression("(a) Est. vs True"~a), 
  x_lab = expression("True"~a),
  y_lab = expression("Estimated"~a),
  theme = mytheme
)
big_b_est <- true_vs_est_scatterplot(
  big_sim$logb, logb_s,
  axis_lim = b_range,
  title = expression("(b) Est. vs True"~b), 
  x_lab = expression("True"~b),
  y_lab = expression("Estimated"~b),
  theme = mytheme
)
big_s_est <- true_vs_est_scatterplot(
  big_sim$logs, logs_s,
  axis_lim = s_range,
  title = expression("(c) Est. vs True"~s), 
  x_lab = expression("True"~s),
  y_lab = expression("Estimated"~s),
  theme = mytheme
)
big_z_est <- true_vs_est_scatterplot(
  big_sim$z_true, z_s,
  axis_lim = z_range,
  title = expression("(d) Est. vs True"~z[10]), 
  x_lab = expression("True"~z[10]),
  y_lab = expression("Estimated"~z[10]),
  theme = mytheme
)
